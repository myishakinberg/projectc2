  ...
  public function processQuery(GetLocationEvent $event) {
    if ($event->getDataSource() == self::sourceId()) {
      ...
      $location->set('originalData', $record)
        ->set('country', isset($record['country']['names'][$lang]) ? $record['country']['names'][$lang] : '')
        ->set('countryCode', isset($record['country']['iso_code']) ? Unicode::strtoupper($record['country']['iso_code']) : '')
        ->set('region', isset($record['subdivisions'][0]['names'][$lang]) ? $record['subdivisions'][0]['names'][$lang] : '')
        ->set('regionCode', isset($record['subdivisions'][0]['iso_code']) ? $record['subdivisions'][0]['iso_code'] : '')
        ->set('city', isset($record['city']['names'][$lang]) ? $record['city']['names'][$lang] : '')
        ->set('zip', isset($record['postal']['code']) ? $record['postal']['code'] : '')
        ->set('latitude', isset($record['location']['latitude']) ? $record['location']['latitude'] : '')
        ->set('longitude', isset($record['location']['longitude']) ? $record['location']['longitude'] : '')
        ->set('timeZone', isset($record['location']['time_zone']) ? $record['location']['time_zone'] : '');
    }
  }
